        ...
        for (propName in srvResults) {
          ...
          if (currentAddresses) { //did the DNS lookup succeed?
            ...
            if (preConfiguredService) {
              //if there is a env configured server AND a suffix AND a slash is missing from both
              //the name and service THEN add it
              if (preConfiguredService.charAt(preConfiguredService.length - 1)!== '/' && 
                suffix && 
                suffix.charAt(0) !== '/') {
                preConfiguredService = preConfiguredService.concat('/');
              }
              result[service.name] = {
